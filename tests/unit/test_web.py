...
def test_extract_single_http_url() -> None:
    ...
    urls = chatbot.utils.web.extract_urls_from_text(text)
...
def test_extract_urls_no_crashes_on_arbitrary_text(text: str) -> None:
    ...
    Args:
        text: Arbitrary text input to test URL extraction robustness.
    """
    urls = chatbot.utils.web.extract_urls_from_text(text)
...
def test_url_regex_pattern() -> None:
    ...
    for url, expected in test_cases:
        match = chatbot.utils.web.URL_REGEX.search(url)
...
async def test_fetch_single_url_success() -> None:
    ...
    content = await chatbot.utils.web.fetch_content_from_urls(["http://example.com"], mock_client)
...
async def test_fetch_multiple_urls_success() -> None:
    ...
    content = await chatbot.utils.web.fetch_content_from_urls(urls, mock_client)
...
async def test_fetch_url_http_error() -> None:
    ...
    mock_response.raise_for_status.side_effect = httpx.HTTPStatusError(
        "404 Not Found", request=unittest.mock.MagicMock(), response=unittest.mock.MagicMock()
    )
    mock_client.get.return_value = mock_response

    content = await chatbot.utils.web.fetch_content_from_urls(["http://notfound.com"], mock_client)
...
async def test_fetch_mixed_success_failure() -> None:
    ...
    success_response.raise_for_status = unittest.mock.MagicMock()

    # Second URL fails
    error_response = unittest.mock.MagicMock()
    error_response.raise_for_status.side_effect = httpx.HTTPStatusError(
        "500 Server Error", request=unittest.mock.MagicMock(), response=unittest.mock.MagicMock()
    )
...
async def test_fetch_empty_urls_list() -> None:
    ...
    content = await chatbot.utils.web.fetch_content_from_urls([], mock_client)
...
async def test_lookup_single_url() -> None:
    ...
    urls, content = await chatbot.utils.web.lookup_http_urls_in_prompt(prompt, mock_client)
...
async def test_lookup_no_crashes_on_arbitrary_prompts(prompt: str) -> None:
    ...
    Args:
        prompt: Arbitrary prompt text to test URL lookup robustness.
    """
    mock_client = unittest.mock.AsyncMock(spec=httpx.AsyncClient)
